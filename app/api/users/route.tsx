...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (id) {
      ...
      const [rows] = await connection.execute('SELECT * FROM users WHERE email = ?', [id]);
    ...
    {
      const [rows] = await connection.execute("SELECT * FROM users WHERE userType = 'mentor'");
      await connection.end();
...
export async function POST(request: Request) {
  try {
    ...
    const { name, type, designation, institute, phone, email, password } = data;

    // Validate input
    if (!name || !type || !designation || !institute || !phone || !email || !password) {
    ...
    const [result] = await connection.execute(
      'INSERT INTO users (name, usertype, designation, institute, phone, email, password) VALUES (?, ?, ?,?,?,?,?)',
