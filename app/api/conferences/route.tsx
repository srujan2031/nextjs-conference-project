...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (id) {
      ...
      const [rows] = await connection.execute('SELECT * FROM conferences WHERE id = ?', [id]);
      await connection.end();

      return NextResponse.json(rows); // Return the single conference
    } else {
      // Fetch all conferences if no ID is provided
      const [rows] = await connection.execute('SELECT * FROM conferences');
      await connection.end();
...
//       'INSERT INTO conferences (name, type, location) VALUES (?, ?, ?)',
//       [name, type, location]
//     );
//     await connection.end();

//     return NextResponse.json({ message: 'Conference added successfully' });
//   } catch (error) {
...
//   try {
//     const data = await request.json();
//     const { id, name, date, location } = data;

//     const connection = await connectToDatabase();
//     const [result] = await connection.execute(
//       'UPDATE conferences SET name = ?, date = ?, location = ? WHERE id = ?',
...
export async function DELETE(request: Request) {
  try {
    ...
    const connection = await connectToDatabase();
    const [result] = await connection.execute('DELETE FROM conferences WHERE id = ?', [id]);
    await connection.end();
